...
*** Current results
As of noon, 25 March 2024, these produce the following results.
...
at the beginning of this document, some are expected.
...
Some reflect the grammar's current restriction to Alloy 4:
- examples/temporal/buffer.als.xml fails to parse (unrecognized ~var~ keyword).
- examples/temporal/leader.als.xml fails to parse (unrecognized ~var~ keyword).
- examples/temporal/leader_events.als.xml fails to parse (unrecognized ~var~ keyword).
- examples/temporal/trash.als.xml fails to parse (unrecognized ~var~ keyword).

Others are not expected and reflect issues:
...
Others have not yet been analyzed.
- examples/puzzles/handshake.als.xml is ambiguous.  May also be
  /int/-related.
- examples/systems/views.als.xml is ambiguous.  Ambiguity involving
  keyword "~this~".

*** Operator clashes (currently wont-fix)

Model ~book/chapter6/hotel4.als~ contains the following fact:
#+begin_example
fact NoIntervening {
	all c: Checkin |
		c.post = last
...
*** Temporal keywords
Currently, any model using temporal logic fails on an unrecognized
~var~ keyword.  Model ~examples/temporal/trash.als~, for example fails
on line 1:
#+begin_example
var sig File {}
#+end_example
